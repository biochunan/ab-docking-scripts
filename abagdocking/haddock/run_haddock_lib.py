...
"""
# TODO: fix the absolute paths in the subprocess.run commands
# *************************************************************************
...
def clean_inputs(antibody, antigen, ab_filename, ag_filename):
    ...
    # TODO: where are these scripts located?
    # TODO: [ ] /home/oliverh/DockingSoftware/pdb-tools/pdbtools/pdb_chain.py
    # TODO: [ ] /home/oliverh/DockingSoftware/pdb-tools/pdbtools/pdb_seg.py
    # NOTE: this requires BiopTools
    subprocess.run(
        [
            f"/home/oliverh/DockingSoftware/pdb-tools/pdbtools/pdb_chain.py {antibody} | /home/oliverh/DockingSoftware/pdb-tools/pdbtools/pdb_seg.py | pdbrenum > {ab_filename}_clean.pdb"
        ],
        shell=True,
    )
    ...
    subprocess.run(
        [
            f"/home/oliverh/DockingSoftware/pdb-tools/pdbtools/pdb_chain.py {antigen} | /home/oliverh/DockingSoftware/pdb-tools/pdbtools/pdb_seg.py > {ag_filename}_clean.pdb"
        ],
        shell=True,
    )
    print("Done")
...
def generate_unambig_tbl(ab_filename):
    ...
    # TODO: [ ] /home/oliverh/DockingSoftware/haddock-tools/restrain_bodies.py
    # TODO: [ ] antibody-antigen-unambig.tbl
    subprocess.run(
        [
            f"~/DockingSoftware/haddock-tools/restrain_bodies.py {ab_clean} > antibody-antigen-unambig.tbl"
        ],
        shell=True,
    )
...
def generate_run_param(ab_filename, ag_filename, OUTPath):
    ...
    lines = [
        "HADDOCK_DIR=/home/oliverh/DockingSoftware/haddock2.4",
        "N_COMP=2",
        f"PDB_FILE1={OUTPath}{ab_filename}_clean.pdb",
        f"PDB_FILE2={OUTPath}{ag_filename}_clean.pdb",
        "PROJECT_DIR=./",
        "PROT_SEGID_1=A",
        "PROT_SEGID_2=B",
        "RUN_NUMBER=1",
...
def get_his_protonation(PDBfile):
    ...
    # TODO: [ ] ~/DockingSoftware/haddock-tools/molprobity.py
    protonation = subprocess.check_output(
        [f"~/DockingSoftware/haddock-tools/molprobity.py {PDBfile}"], shell=True
    )
    # Decode output
...
def extract_best_results(inputfilename):
    ...
    subprocess.run(
        [
            f"cp ./run1/structures/it1/{best_result_nw} ./{inputfilename}_Haddock_nowaters_result.pdb"
        ],
        shell=True,
    )
    ...
    subprocess.run(
        [
            f"cp ./run1/structures/it1/water/{best_result_w} ./{inputfilename}_Haddock_waters_result.pdb"
        ],
        shell=True,
    )
